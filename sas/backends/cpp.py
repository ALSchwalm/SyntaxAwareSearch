...
def resolve_ast(tu, ast, config):
    """ Yields cursors matching the pattern in 'ast'
    """
    if isinstance(ast, Function):
        for cursor in resolve_function(tu, ast, config):
            yield cursor
    elif isinstance(ast, Variable):
        for cursor in resolve_variable(tu, ast, config):
            yield cursor
    elif isinstance(ast, Class):
        for cursor in resolve_class(tu, ast, config):
            yield cursor
    elif isinstance(ast, Search):
        for cursor in resolve_basic_search(tu, ast, config):
            yield cursor
...
def resolve_qualifiers(tu, qualifiers, config):
    if not qualifiers:
        for cursor in get_cursors(tu, filename=config.filename):
            yield cursor
    else:
            ...
            if len(qualifiers) == 0:
                ...
                for child in cursor.get_children():
                    ...
                    for inner in recurse_qualifiers(child, qualifiers[1:]):
                        yield inner
        for cursor in get_root_cursors(tu, qualifiers[0], config.filename):
...
def resolve_function(tu, function, config):
    allowed_kinds = []
    ...
    for cursor in resolve_qualifiers(tu, function.qualifiers, config):
...
def resolve_variable(tu, variable, config):
    allowed_kinds = []
    if config.mode & Config.MATCH_MODE.DECLARATION:
    ...
    for cursor in resolve_qualifiers(tu, variable.qualifiers, config):
        if match(variable.name, cursor.spelling) and \
           matches_by_kinds(cursor, variable, allowed_kinds):
            yield cursor


def resolve_class(tu, class_t, config):
    allowed_kinds = []
    if config.mode & Config.MATCH_MODE.DECLARATION:
    ...
    if config.mode & Config.MATCH_MODE.EXPRESSION:
        allowed_kinds += []
    for cursor in resolve_qualifiers(tu, class_t.qualifiers, config):
        if match(class_t.name, cursor.spelling) and \
           cursor.kind in allowed_kinds:
            yield cursor


def resolve_basic_search(tu, search, config):
    for cursor in resolve_qualifiers(tu, search.qualifiers, config):
